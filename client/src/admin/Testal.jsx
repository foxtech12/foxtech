...
const Testimonial = () => {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      const response = await axiosInstance.delete(`/api/test/delete-all`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchSuggestions = async () => {
    try {
      setLoading(false);
      const response = await axiosInstance.get(`/api/test/get-event`);
  ...
  const deleteSuggestion = async (id) => {
    ...
    try {
      await axiosInstance.delete(`/api/test/delete/${id}`);
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      await axiosInstance.put(
        `/api/test/update/${selectedSuggestion._id}`,
        formData,
      ...
      <div className="p-4 sm:ml-64">
        <div className="p-4 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
          <div className="flex flex-col sm:flex-row justify-end items-center mb-4 space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            >
              {allDeleted ? "All Items Deleted" : "Delete All Testimonial"}
            </button>
          </div>
          {/* Table content */}
